...
async function ensureSlot(data: string, periodo: string, capacidade = Number(process.env.AGENDA_CAPACIDADE_PADRAO || 3)) {
  const existing = await findSlot(data, periodo);
  if (existing) return existing;
  return createSlot({ data, periodo, capacidade, reservados: 0, bloqueado: 0 });
}

async function getActiveQuoteForCustomer(customerId: number, attendance: any = null) {
...
export function getToolDefinitions(): ToolDef[] {
  return [
    ...
    },
    {
      ...
      description: 'Retorna (ou cria) atendimento atual do cliente.',
      inputSchema: { type: 'object', properties: { customerId: { type: 'number' } }, required: ['customerId'] },
      outputSchema: { type: 'object' },
      handler: async ({ customerId }) => getOrCreateAttendance(Number(customerId)),
    },
    {
      ...
      handler: async ({ customerId, message }) => {
        await saveMessage({ customerId: Number(customerId), direction: 'entrada', content: asText(message) });
        return { ok: true };
      },
    },
    {
      ...
      outputSchema: { type: 'object' },
      handler: async ({ customerId, message }) => {
        await saveMessage({ customerId: Number(customerId), direction: 'resposta', content: asText(message) });
        return { ok: true };
      },
    },
    {
      ...
      handler: async ({ customerId, state }) => {
        assertState(state);
        return updateAttendance(Number(customerId), { estado: state });
      },
    },
    {
      ...
      handler: async ({ customerId, mode, manualReason = null, preserveState = true }) => {
        assertMode(mode);
        const attendance = await getOrCreateAttendance(Number(customerId));
        const nextPayload: any = { modo: mode, manual_motivo: manualReason };
        ...
        return updateAttendance(Number(customerId), nextPayload);
      },
    },
    {
      name: 'analyze_damage_image',
      description: 'Armazena imagem recebida e executa análise por embeddings.',
      inputSchema: {
        type: 'object',
        properties: {
          phone: { type: 'string' },
    ...
    },
    {
      ...
      description: 'Cria orçamento e vincula ao atendimento atual.',
      inputSchema: {
        type: 'object',
        properties: {
          customerId: { type: 'number' },
          imageId: { type: 'number' },
          quotePayload: { type: 'object' },
        },
        required: ['customerId', 'quotePayload'],
      },
      outputSchema: { type: 'object' },
      handler: async ({ customerId, imageId = null, quotePayload }) => {
        const quote = await createQuote({ customerId: Number(customerId), imageId, ...quotePayload });
        await updateAttendance(Number(customerId), { orcamento_id_atual: quote.id });
        return quote;
      },
    },
    {
      ...
      outputSchema: { type: 'object' },
      handler: async ({ quoteId, updates }) => updateQuote(Number(quoteId), updates),
    },
    {
      name: 'reserve_schedule_slot',
      description: 'Reserva agenda e persiste no orçamento atual.',
      inputSchema: {
        type: 'object',
      ...
      handler: async ({ customerId, date, period }) => {
        ...
        const attendance = await getOrCreateAttendance(Number(customerId));
        const quote = await getActiveQuoteForCustomer(Number(customerId), attendance);
        ...
        await updateAttendance(Number(customerId), {
          estado: ATTENDANCE_STATE.CLOSED,
          orcamento_id_atual: quote.id,
        });
    ...
    },
    {
      ...
      description: 'Libera slot reservado.',
      inputSchema: {
        type: 'object',
        properties: { customerId: { type: 'number' }, date: { type: 'string' }, period: { type: 'string' } },
        required: ['customerId', 'date', 'period'],
      ...
      handler: async ({ customerId, date, period }) => {
        ...
        if (quote && quote.slot_data === date && quote.slot_periodo === normalizedPeriod) {
          ...
          await updateAttendance(Number(customerId), { estado: ATTENDANCE_STATE.WAITING_SCHEDULE });
        }
    ...
    },
    {
      ...
      description: 'Escala atendimento para humano (modo MANUAL + estado HUMAN_HANDOFF).',
      inputSchema: { type: 'object', properties: { customerId: { type: 'number' }, reason: { type: 'string' } }, required: ['customerId'] },
      outputSchema: { type: 'object' },
      handler: async ({ customerId, reason }) => updateAttendance(Number(customerId), {
    ...
    },
    {
      ...
      description: 'Retoma atendimento automático preservando contexto.',
      inputSchema: { type: 'object', properties: { customerId: { type: 'number' } }, required: ['customerId'] },
      outputSchema: { type: 'object' },
      handler: async ({ customerId }) => {
        ...
        return updateAttendance(Number(customerId), {
    ...
    },
    {
      ...
      handler: async ({ customerId }) => {
        ...
        return updateAttendance(Number(customerId), { estado: ATTENDANCE_STATE.CLOSED });
      },
    },
    {
